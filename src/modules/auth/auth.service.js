...
export const register = async ({ username, email, password }) => {
  ...
  const { rawToken: rawVerificationToken, hashedToken: hashedVerificationToken } = generateTokenPair();
  ...
  await redis.set(`verify:${hashedVerificationToken}`, user._id.toString(), "EX", 60 * 5);
...
export const login = async ({ email, password }) => {
  ...
  const { rawToken: refreshToken, hashedToken: hashedRefreshToken } = generateTokenPair();
...
export const refresh = async (refreshToken) => {
  ...
  const { rawToken: newRefreshToken, hashedToken: newHashedToken } = generateTokenPair()

  await redis.set(`rt:${newHashedToken}`, user._id.toString(), "EX", 60 * 60 * 24 * 7);
